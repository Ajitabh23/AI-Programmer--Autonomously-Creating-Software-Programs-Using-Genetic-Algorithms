    ...
    for i in range(0, len(child)):
        ...
        if MUTATION_RATE >= random.random():
            ...
            if mutation_type ==1:
                ...
                add_instruction(child)
            elif mutation_type ==3:
                delete_instruction(child, i)
...
def main() :
    ...
    while(1):
        ...
        if generations>=MAX_GEN_COUNT:
            break
        worst_program_index = 0
        best_program = score_population(programs, scores, worst_program_index)
        for i in range(0, int(POP_SIZE/2)):
            # select parents
            parent1 = select_parent(programs, scores)
            parent2 = select_parent(programs, scores, parent1)
            # crossover
            child1, child2 = crossover(parent1, parent2)
            children[i*2] = child1
            children[i*2+1] = child2
        
        load_new_generation(programs, children)
        if not genome_exists(best_program, programs):
        ...
        if not(generations % DISPLAY_RATE):
            ...
            output_print = brainfuck.evaluate(best_program)
            print("Output: ")
            print(output_print)

            if output_print == GOAL_OUTPUT and (not keep_going) :
